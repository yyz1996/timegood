...
public class draw_corr_dist {
    ...
    static int time_num=delta_t.length;//不同时间粒度的数目
    static int product_num= product.length;//不同商品种类的数目
    static double corr[][]=new double [time_num][product_num];//每列存储一种商品的标准化相关系数
    static double dist[][]=new double [time_num][product_num];//每列存储一种商品的标准化欧氏距离

    public static void main(String[] args)
    {
        for(int i=0;i<product_num;i++)
        {
            String filename_corr="D:\\Java\\Test\\standard_corr_"+product[i]+".xls";
            String filename_dist="D:\\Java\\Test\\standard_dist_"+product[i]+".xls";
            double temp_corr[]=new double[time_num];
            double temp_dist[]=new double[time_num];
            read_data(filename_corr,temp_corr);
            read_data(filename_dist,temp_dist);
            for(int j=0;j<time_num;j++)
            {
                corr[j][i]=temp_corr[j];
                dist[j][i]=temp_dist[j];
            }
        }

        //绘制相关系数曲线
        String path_corr="D:\\picture\\all_product_standard_corr_dist\\product_standard_corr.jpg";
        DefaultCategoryDataset ds_corr = (DefaultCategoryDataset) getDataSet(corr,product,delta_t,product_num,time_num);
        getLineChart(ds_corr,path_corr,"时间粒度","标准化相关系数","标准化相关系数与时间粒度图",false);

        //绘制欧式距离曲线
        String path_dist="D:\\picture\\all_product_standard_corr_dist\\product_standard_dist.jpg";
        DefaultCategoryDataset ds_dist = (DefaultCategoryDataset) getDataSet(dist,product,delta_t,product_num,time_num);
        getLineChart(ds_dist,path_dist,"时间粒度","标准化欧氏距离","标准化欧氏距离与时间粒度图",false);
    }
