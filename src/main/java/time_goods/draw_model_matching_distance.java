...
public class draw_model_matching_distance {
    ...
    static double model_matching_distance[][]=new double [time_num][product_num];//每列存储一种商品的标准化动态时间弯曲距离

    public static void main(String[] args) {
        for (int i = 0; i < product_num; i++) {
            String filename_model_matching = "D:\\Java\\Test\\standard_model_matching_distance_" + product[i] + ".xls";
            double temp_model_matching[] = new double[time_num];
            read_data(filename_model_matching, temp_model_matching);

            for (int j = 0; j < time_num; j++) {
                model_matching_distance[j][i] = temp_model_matching[j];
            }
        }

        //绘制动态时间弯曲距离曲线
        String path_model_matching = "D:\\picture\\all_product_standard_model_matching_distance\\product_standard_model_matching_distance.jpg";
        DefaultCategoryDataset ds_model_matching = (DefaultCategoryDataset) getDataSet(model_matching_distance, product, delta_t, product_num, time_num);
        getLineChart(ds_model_matching,path_model_matching, "时间粒度", "标准化模式匹配距离", "标准化模式匹配距离与时间粒度图", false);
    }
