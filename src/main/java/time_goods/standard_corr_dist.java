...
public class standard_corr_dist {
    //时间粒度
    ...
    public static void main(String[] args)
    {
        ...
        String standard_dist_filename="standard_dist_"+product+".xls";
        String standard_corr_filename="standard_corr_"+product+".xls";
        write_standard_data(Dist,"时间粒度","标准化欧氏距离",standard_dist_filename);
        write_standard_data(Corr,"时间粒度","标准化相关系数",standard_corr_filename);

        //绘制欧式距离曲线
        String path_dist = "D:\\picture\\standard_dist\\standard_dist_"+product+".jpg";
        DefaultCategoryDataset ds_dist = (DefaultCategoryDataset) getDataset_corr_dist(Dist,product);
        getLineChart(ds_dist,path_dist,"时间粒度","标准化欧氏距离","标准化欧氏距离与时间粒度图",true);//num_label为是否显示折点数据

        //绘制相关系数曲线
        String path_corr = "D:\\picture\\standard_corr\\standard_corr_"+product+".jpg";
        DefaultCategoryDataset ds_corr = (DefaultCategoryDataset) getDataset_corr_dist(Corr,product);
        getLineChart(ds_corr,path_corr,"时间粒度","标准化相关系数","标准化相关系数与时间粒度图",true);
        ...
        for(int i=0;i<delta_t.length;i++)
        {
            ...
            String path_price="D:\\picture\\price\\price_"+product+"_"+i+".jpg";
            String xlabel_price="时间";
            String ylabel_price="价格";
            String title_price="时间粒度为"+delta_t[i]+"时"+product+"十年价格走势图";
            DefaultCategoryDataset ds_price=(DefaultCategoryDataset) getDataset_price(price_time_granularity,price,delta_t[i],time_num,year);
            getLineChart(ds_price,path_price,xlabel_price,ylabel_price,title_price,false);
        }

        //根据阈值选定合适的时间粒度
        int time_granularity=1;
        time_granularity=recommend_time(Corr,delta_t,Threshold);
    ...
    { //将生成的数据输出在项目的当前目录下
        try
        {
            ...
            int row_num=delta_t.length;//标准化欧式距离数组行数
